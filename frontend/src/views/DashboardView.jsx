...
import { Terminal, Database, TrendingUp, Users, ArrowRight, ShieldCheck, Zap, Activity, Cpu } from 'lucide-react';
...
export default function DashboardView({ onStartInterview }) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="bg-white/80 dark:bg-[#0c1427]/80 backdrop-blur-xl border border-slate-200 dark:border-[#17233f] rounded-3xl p-6 shadow-sm space-y-4">
              ...
              <button
                ...
                className={`p-3.5 rounded-2xl text-left border transition-all ${
                  isSelected
                    ...
                    : 'bg-slate-50/80 dark:bg-[#0f1a33]/60 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-[#1a294c] hover:border-indigo-400 dark:hover:border-indigo-500'
                }`}
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="font-black text-xs md:text-sm">{tier.label}</span>
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${isSelected ? 'bg-white/20 text-white' : 'bg-slate-200 dark:bg-[#162444] text-slate-500 dark:text-slate-400'}`}>
                    {tier.sub}
                  </span>
                </div>
          ...
          {tracks.map((track) => {
            ...
            return (
              <div
                ...
                className="group relative bg-white dark:bg-[#0c1427]/90 border border-slate-200 dark:border-[#17233f] hover:border-indigo-500 dark:hover:border-indigo-500 rounded-3xl p-7 cursor-pointer transition-all duration-300 shadow-sm hover:shadow-2xl hover:-translate-y-1 space-y-4 overflow-hidden backdrop-blur-xl"
              >
                {/* Glow Accent */}
